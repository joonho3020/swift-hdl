...
let clock = Wire(false, name: "clock")
let counter = Reg(HWUInt(0, width: 4), name: "counter")
counter.setClock(clock)

for cycle in 0..<5 {
    print("  Cycle \(cycle): Counter = \(counter.value.value)")
    let newValue = counter.value + HWUInt(1, width: 4)
...
let a = HWUInt(5, width: 8)
let b = HWUInt(3, width: 8)
let result = a + b
...
let inputWire = Wire(HWUInt(10, width: 4), name: "input")
let outputWire = Wire(HWUInt(0, width: 4), name: "output")
...
let x = HWUInt(7, width: 4)
let y = HWUInt(3, width: 4)
